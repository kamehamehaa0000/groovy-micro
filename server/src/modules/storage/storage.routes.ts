...
export const storageRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    { preHandler: [requireAuth] },
    async (request, reply) => {
      const releases = await storageService.getUserLockerReleases(request.user.id);
  ...
  const handleDeletePersonalSong = async (request: any, reply: any) => {
    ...
    try {
      const result = await storageService.deletePersonalSong(request.user.id, id);
  ...
  const handleDeletePersonalRelease = async (request: any, reply: any) => {
    const { id } = request.params as { id: string };
    try {
      const result = await storageService.deletePersonalRelease(request.user.id, id);
      return reply.status(200).send(result);
    } catch (err: any) {
      return reply.status(404).send({
        statusCode: 404,
