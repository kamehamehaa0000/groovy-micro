...
function PlaylistsIndexComponent() {
  ...
  const { isPlaylistSaved, toggleSavePlaylist, hydratePlaylists } =
    usePlaylistsStore()
  const openCreateModal = useCreatePlaylistModalStore((s) => s.openModal)
  ...
  // Automatically open modal if ?create=true
  useEffect(() => {
    if (searchParams.create && isAuthenticated) {
      openCreateModal()
    }
  }, [searchParams.create, isAuthenticated, openCreateModal])
